...
const IG_URL    = 'https://www.instagram.com/ml.editing.media?igsh=MXhzbjhjeXo2eG5vdA%3D%3D&utm_source=qr';
const TALLY_URL = 'https://tally.so/r/MeNzj0';
const BLEND: React.CSSProperties = { mixBlendMode: 'screen', filter: 'brightness(1.1)' };
...
export function Navbar() {
  ...
  return (
    <>
      {/* Full-screen backdrop — closes dropdown when clicking outside */}
      {showContact && (
        <div
      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            <a href="#portfolio" onClick={scrollToPortfolio} className="text-sm font-semibold tracking-widest uppercase text-white/70 hover:text-white transition-colors">
              Portfolio
            </a>
            <div className="relative z-50">
              ...
              >
                Anfragen
                <svg className={`w-3 h-3 transition-transform duration-200 ${showContact ? 'rotate-180' : ''}`} viewBox="0 0 12 12" fill="currentColor"><path d="M6 8L1 3h10L6 8z"/></svg>
              </button>

              {showContact && (
                <div className="absolute top-full right-0 mt-3 w-64 rounded-sm overflow-hidden" style={{ background:'rgba(14,14,14,0.98)', border:'1px solid rgba(255,255,255,0.1)', boxShadow:'0 20px 60px rgba(0,0,0,0.7)', backdropFilter:'blur(20px)' }}>
                  <p className="text-[10px] font-semibold tracking-widest uppercase text-white/30 px-5 pt-4 pb-2">Anfrage senden über</p>
                  <div className="flex flex-col gap-1 px-3 pb-3">
                    <a href={IG_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3.5 px-3 py-3 rounded-sm hover:bg-white/5 transition-colors group">
                      <div className="w-9 h-9 rounded-sm bg-black/40 flex items-center justify-center shrink-0">
                        <img src={igLogo} alt="Instagram" className="w-7 h-7 object-contain" style={BLEND} />
                      </div>
                      <div>
                    ...
                    <a href={TALLY_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3.5 px-3 py-3 rounded-sm hover:bg-white/5 transition-colors group">
                      <div className="w-9 h-9 rounded-sm bg-black/40 flex items-center justify-center shrink-0">
                        <img src={tallyLogo} alt="Tally" className="w-7 h-7 object-contain" style={BLEND} />
                      </div>
                      <div>
          ...
          <div className="flex md:hidden items-center gap-3">
            <div className="relative z-50">
              ...
              >
                Anfragen
                <svg className={`w-2.5 h-2.5 transition-transform duration-200 ${showContact ? 'rotate-180' : ''}`} viewBox="0 0 12 12" fill="currentColor"><path d="M6 8L1 3h10L6 8z"/></svg>
              </button>
            ...
            <button onClick={() => setMenuOpen(o => !o)} className="w-9 h-9 flex flex-col items-center justify-center gap-[5px]" aria-label="Menü">
